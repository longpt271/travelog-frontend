    ...
    return (
        <div id="admin">
            ...
            <div className="title text-center mt-3">
                {state.check === 'trong' ? <h4>Quản lý tour trong nước</h4> : <h4>Quản lý tour nước ngoài</h4>}
                <div className="hr"></div>
            </div>
                    ...
                    <Table columns={columns} dataSource={tourtrongnuoc.map((ok, index) => (
                        {
                            key: index + 1,
                            name: <Link to={`${match.url}/chitiettour/${ok.id}`}>{ok.name}</Link>,
                            avatar: <Image src={ok.avatar} width="150px" height="200px" border-radius="8px" alt="" />,
                            status: <div className="action">{ok.status === 1 ? <span onClick={() => { handleStatus(ok.status, ok.id) }}><i className="far fa-eye text-primary"></i></span> : <span onClick={() => handleStatus(ok.status, ok.id)}><i className="far fa-eye-slash"></i></span>}</div>,
                            action:
                                <div className="action">
                                    <Popconfirm title="Bạn có muốn sửa？" onConfirm={() => { hangdleEdit(ok.id) }} icon={<QuestionCircleOutlined style={{ color: 'green' }} />}>
                                        <i className="far fa-edit mr-4" style={{ cursor: "pointer" }}></i>
                    ...
                    <Table columns={columns} dataSource={tournuocngoai.map((ok, index) => (
                        {
                            key: index + 1,
                            name: <Link to={`${match.url}/chitiettour/${ok.id}`}>{ok.name}</Link>,
                            avatar: <Image src={ok.avatar} width="150px" height="200px" alt="" />,
                            status: <div className="action">{ok.status === 1 ? <span onClick={() => { handleStatus(ok.status, ok.id) }}><i className="far fa-eye text-primary"></i></span> : <span onClick={() => handleStatus(ok.status, ok.id)}><i className="far fa-eye-slash"></i></span>}</div>,
                            action:
                                <div className="action">
                                    <Popconfirm title="Bạn có muốn sửa？" onConfirm={() => { hangdleEdit(ok.id) }} icon={<QuestionCircleOutlined style={{ color: 'green' }} />}>
